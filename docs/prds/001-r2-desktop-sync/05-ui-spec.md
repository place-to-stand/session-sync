...
The invite link (`https://<deployment>.convex.site/invite/{token}`) is generated and redeemed via Convex HTTP actions:

1. Admin clicks "Invite Partner" -> Tauri (Rust backend) calls Convex HTTP action `createInvite` via a Rust HTTP client -> returns a signed token (UUID + HMAC signature, expires in 7 days)
2. Admin shares the link (copy/paste, iMessage, email)
3. Invited engineer pastes the link into the SessionSync setup wizard
4. The desktop app extracts the token and calls Convex HTTP action `redeemInvite` (passing `token`, `machineName`, `userName`) via the Rust HTTP client -> validates signature + expiry + single-use check -> creates user + machine records -> returns Convex auth credentials
5. Auth credentials stored in macOS Keychain. Machine is now registered.

Both HTTP actions run entirely on the Convex backend and use a signing secret stored in Convex environment variables; the desktop app never sees the secret.

**Security note for MVP:** Invites are strictly single-use bearer tokens with a 7-day TTL. Server-side, each invite stores `createdAt`, `expiresAt`, optional `revokedAt`, and `redeemedAt`. `redeemInvite()` must:
- Reject tokens that are expired, revoked, or already redeemed.
- On first successful redemption, atomically set `redeemedAt` and bind the invite to the created user/machine.
- Allow the admin to revoke unused invites (sets `revokedAt`), after which redemption fails even if the link is still in circulation.
